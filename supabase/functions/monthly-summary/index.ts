...
Deno.serve(async () => {
  ...
  const { data: members } = await supabase.from('mentis_members').select('id,name,organization_id,customer_id');
  let sent = 0;
  for (const m of members ?? []) {
    const { data: paused } = await supabase.from('mentis_enrollments').select('id')
      .eq('member_id', m.id).eq('status', 'paused').limit(1);
    if (paused?.length) continue;
    const { data: records } = await supabase.from('mentis_attendance_records').select('status,session_id')
      .eq('member_id', m.id).gte('recorded_at', first.toISOString()).lte('recorded_at', last.toISOString());
    const total = records?.length ?? 0;
    if (!total) continue;
    const attended = (records ?? []).filter((r) => r.status === 'present' || r.status === 'late').length;
    const pct = Math.round((attended / total) * 100);
    const { data: customer } = await supabase.from('mentis_customers').select('email,name').eq('id', m.customer_id).single();
    if (!customer?.email) continue;
    const { data: debits } = await supabase.from('mentis_customer_charges').select('amount_cents')
