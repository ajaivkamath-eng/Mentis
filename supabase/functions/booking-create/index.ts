...
Deno.serve(async (req) => {
  ...
  if (req.method === 'POST') {
    ...
    const { data: slot } = await supabase.from('booking_slots').select('*').eq('id', slotId).single();
    if (!slot || slot.status !== 'open') return Response.json({ error: 'slot is not open' }, { status: 400, headers: cors });
    const { data: member } = await supabase.from('members').select('id,customer_id,organization_id').eq('id', memberId).single();
    ...
    const { data: staffing } = await supabase.from('session_staffing').select('session_id,sessions(start_at,end_at)').eq('staff_id', slot.coach_id);
